...
python scripts/jev_advise.py --chat "<会话名>" --cand A --cand B --cand C   # 3 条候选排序
```

输出 `后果风险(0-9) / 上下文贴合 / 对方真正需要 / 敏感类别 / 是否满足需要 / 候选排序`，约 1 秒。退出码 `0` 可发、`3` 建议先别发、`4` **判断层不可用（不算通过）**。

三条纪律（实测踩出来的）：**① 它是提醒，不是裁决**（同一批候选里它把使用者自己选的那条排到过 0.04）；**② 0.35-0.6 是「不确定」不是告警**（把 <0.5 当告警会“狼来了”，只有强信号才建议别发）；**③ 口径要按场景调**——亲密关系型的「道歉-试探-通牒」rubric 用到工作/朋友闲聊上会把一句“再约”判成 4/9，本工具已改为「这条回复发出去的后果风险」。
