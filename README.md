...
**不要试图「把微信搬到屏幕外」**：库用 `uiautomation` 的 `Control.Click()`（即 `SetCursorPos` + `mouse_event` 物理点击）打开会话，窗口一旦不在屏幕上，点击坐标无效、打开会话必然失败（实测 22.8s 失败）。也别在外层「记住窗口几何再还原」——库自身会在任务中改写几何，实测还原后窗口落在了别的坐标。
