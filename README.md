...
**第二层·判断层（可选，TypeSafe Jev）**：`scripts/jev_advise.py` 给「这条回复该不该发」做结构化判断——机械闸门只能防「发错人」「发重复」，防不了语义问题。**它只适合一个位置：使用者要你起草或建议回复时**（这时选择本来就归他，Jev 的排序与「对方真正需要什么」帮他省事）。使用者已经口述了具体文本时**不要**跑——那是他的决定；实测同一批候选里 Jev 把他自己选的那条排到过 0.04，拿它质疑他的措辞是本末倒置。

```bash
# 判断一条拟发文本（默认读该会话最近 8 条作上下文）
python scripts/jev_advise.py --chat "<会话名>" --text "<拟发文本>" --relationship "<关系描述>"
# 给 3 条候选排序（候选由人或生成模型给出，Jev 只排序）
python scripts/jev_advise.py --chat "<会话名>" --cand A --cand B --cand C
```

回答：`后果风险(0-9) / 上下文贴合 / 对方真正需要 / 敏感类别 / 是否满足需要 / 候选排序`，约 1 秒。退出码 `0` 可发、`3` 建议先别发、`4` **判断层不可用（不算通过）**。需要 `TYPESAFE_API_KEY` 或 `~/.pi/.secrets/typesafe.key`；没配时明确报退出码 4，不静默跳过。

三条使用纪律（都是实测踩出来的）：

1. **它是提醒，不是裁决。** 同一批候选里 Jev 把使用者自己选的那条排到了最后（0.04），置信度只有 0.38。给使用者看「Jev 认为…」，不要替他决定。
2. **0.35-0.6 是「不确定」，不是告警。** Jev 对是非题给的是概率；早期把 <0.5 当告警，结果“狼来了”。只有强信号才建议别发（风险≥6、贴合<0.35、需要<0.35、内容含金额/凭证/个人资料）。
3. **口径要按场景调。** 判断模型的十档 rubric 与题目措辞直接决定它判得准不准：本是亲密关系型的「道歉-试探-通牒」口径，用到工作/朋友闲聊上会把一句“再约”判成 4/9；本工具已改成「这条回复发出去的后果风险」。要更准，用自己的真实对话做一批标注校准（`--json` 的输出可直接当标注料）。

> 设计借鉴自 [jev-chat-JARVIS](https://github.com/Finderchangchang/jev-chat-JARVIS)（Android 端对话副驾）：把「生成」与「判断」分工，判断交给快速判断模型，人保留最终发送权。
