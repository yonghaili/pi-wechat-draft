...
## 已知弱点：库的“打开会话”会长时间级联重试

上游 `wechatauto` 打开会话那一步在本机实测会偶发进入长级联（同一会话：一次 **63.5s** 失败、下一次 **4.3s** 成功；历史最坏占用前台 **75s**）。机制已确认：

1. 它用 `_paste_into`（`ctrl.Click()` + 剪贴板 + SendKeys）写搜索框 —— 点击 + 键盘 + 剪贴板，三个易碎环节；
2. 写失败时搜索框里是**上次的旧词**，于是点到了别的会话（日志实录：目标是“文件传输助手”，现场读到的是‘贾冰*妈’），校验失败；
3. 接着走侧栏 OCR 与搜索兜底，整条链**没有外部超时**，所以一次“打不开”能耗掉一分钟前台。

**试过并否定的无点击替代**（别重复走）：把搜索框改成 UIA `ValuePattern` 直写、结果项改成 `Invoke`/`SelectionItem` —— 搜索命中和 Invoke 都成功，但**搜索结果打开的是独立聊天窗口**（库自己的 docstring 也写了这件事），主窗输入框不变、校验必然失败，于是白花 6.8s 再回退库的路径，**净负收益**，已删除。另：`describe_layout()` 的 `session_list` 只是个字符串描述，侧栏项不是可枚举的 UIA 列表项（库因此用 OCR）。

**要根治**得自己实现侧栏选择（UIA 找不到抓手时只能自搞 OCR/坐标，或接受现状），需要单独立项 + A/B。

**当前缓解**：失败是**安全**的（不写入、不发送、日志留证）；静默档把这类尝试限制在你不操作电脑时；重试通常一次就成。
