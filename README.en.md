...
The checker deliberately contains no personal names, so it can never become the leak. CI runs it on every push via [`.github/workflows/selfcheck.yml`](.github/workflows/selfcheck.yml). (Note: pushing a workflow file requires a token with the `workflow` scope — GitHub rejects the push otherwise; grant it with `gh auth refresh -s workflow`.)
